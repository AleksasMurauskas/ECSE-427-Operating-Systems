...
void *write_func(void *arg) {
	int loc, w= 0;
	while(w<write_loops){ //loops 
		sem_wait(&starvation_cure); //Add thread to a queue to avoid starvation
		clock_t writeStartTime =clock(); //beging taking time
		sem_wait(&rw_mutex);
		clock_t writeEndTime = clock();  //stop keeping time
		clock_t elapsed_time = writeEndTime- writeStartTime; //update total time write has been waiting
		...
		sem_post(&starvation_cure); //Play the next thread
		//Write Occurs 
		loc = glob;
...
void *read_func(void *arg) {
	...
	while(r<read_loops){
		...
		if(readCount==1){ //lock for both reader and writer
			sem_wait(&rw_mutex);
		}
		sem_post(&starvation_cure);  //Play the next thread
		sem_post(&mutex); //unlock the file
		clock_t readEndTime = clock();
		clock_t elapsed_time = readEndTime- readStartTime;
		long elapsed_microsec = elapsed_time*1000000/CLOCKS_PER_SEC;
		...
		//printf("Start:%ld\n",elapsed_microsec);
		loc = glob;
    	loc= loc; //reads the global value 
   		glob = loc;
   		printf("%d\n", glob);
...
int main(int argc, char *argv[]) {
	...
	write_loops=inputw; //30
	read_loops=inputr; //60
	int loops=0;
	...
	if (sem_init(&mutex, 0, 1) == -1) {
  	...
  	if (sem_init(&starvation_cure, 0, 1) == -1) {
    	printf("ERROR: init semaphore\n"); //Check for error
   		exit(1);
  	}
	...
	for(int x =0; x<read_num;x++){
		error_val = pthread_create(&read_arr[x], NULL, read_func, &loops);
	...
	for(int y=0;y<writer_num;y++){
		error_val = pthread_create(&write_arr[y], NULL, write_func, &loops);
  		if (error_val != 0) {//Check for error
